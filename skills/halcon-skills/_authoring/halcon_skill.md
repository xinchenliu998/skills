...
## Reference files
Local references (本 skill 同目录的 `_authoring/` 下, 均为本 skill 库内自带):
1. [`hdev_format_reference.md`](hdev_format_reference.md) — `.hdev` XML schema (all tags, attributes, `<interface>` declarations, `<docu>` blocks), `.hdvp`/`.hdpl` procedure formats.
2. [`hscript_format_reference.md`](hscript_format_reference.md) — `.hscript` plain-text format, procedure syntax (`proc`/`endproc`, `public`), `from ... import *` mechanism, and differences from `.hdev`.
3. [`hrun_reference.md`](hrun_reference.md) — `hrun` CLI help, usage patterns, and authoring guidelines for headless `.hdev` execution.
4. [`hscriptengine_reference.md`](hscriptengine_reference.md) — `hscriptengine` CLI help, usage patterns, authoring guidelines, and comparison with `hrun`.

HALCONROOT documentation (外部——指实际 HALCON 安装目录, 非本 skill 库内容; 在 `HALCONROOT/doc/`):
...
### `HALCONROOT`（外部环境变量——指向用户机器上的 HALCON 安装）
...
### `HALCONEXAMPLES`（外部环境变量——指向用户机器上的 HALCON 示例目录）
